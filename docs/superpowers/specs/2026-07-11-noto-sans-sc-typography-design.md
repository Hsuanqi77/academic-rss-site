...
- 日期：2026-07-11
- 状态：字体方案与 13px 技术标签方案已获用户口头批准，等待规范复核
...
6. 提升眉题和筛选序号等技术标签的可读性。

## 3. 非目标

- 不修改文章数据、RSS 抓取或数据库结构。
- 除已批准的技术标签字号外，不改变页面的间距、颜色、卡片结构、响应式断点或交互逻辑。
- 不替换技术标记使用的等宽字体。
...
| 眉题、筛选序号 | `ui-monospace, Consolas, monospace`，10px | 保留等宽字体，改为 13px |
| 右侧版本信息、搜索标签、年份、页脚等其他小型标记 | 当前各自字体与字号 | 保持不变 |
...
Noto Sans SC Variable 的可变字重轴覆盖上述三个字重。现有采用 700 的标题保持视觉层级；实施时只在界面控件缺少明确层级的位置补充 500。除 5.1 节明确指定的技术标签外，不改变其他元素的字号和行高。

### 5.1 已批准的技术标签字号

用户在本地对比 11px、12px 和 13px 后选择 13px。以下两个选择器统一从 10px 调整为 13px：
...
上述标签继续使用 `ui-monospace, Consolas, monospace`、700 字重、1.2 行高、`.16em` 字距和大写转换。该字号在桌面与移动端一致，不新增响应式覆盖。

本次不放大 `.edition`、`.search-label`、`.brand small`、`.badge`、`.chip` 或页脚文字；这些元素保持当前字号和视觉层级。
...
- `docs/styles.css`：将 `.eyebrow` 与 `.filter-index` 从 10px 调整为 13px，其他小型标记不变。
...
7. `.eyebrow` 与 `.filter-index` 的计算字号为 13px；其他明确排除的小型标记保持原字号。
...
6. 13px 技术标签在筛选栏、页面标题区和信息区中不与相邻标题或控件重叠。
