...
`feeds.yml` 是 RSS 来源的唯一来源。添加或修改期刊时，编辑其 `feeds` 列表，每项包含：
...
只使用出版商公开 RSS，不用网页抓取代替 RSS，也不绕过登录、订阅或机构访问控制。修改后按第 7 节重新生成“说明”区并执行同步检查，然后运行测试；如果需要验收真实抓取，再执行两轮 `update.ps1`。

## 7. 维护标签与“说明”区

`topics.yml` 是标签与关键词的唯一来源，同时定义 8 个一级方向、56 个精细标签、英文关键词及可选的上下文门槛。一级方向只用于组织“说明”内容，不写入数据库，也不成为筛选条件；精细标签是论文实际的自动分类和筛选单位。

分类使用论文标题和摘要，不区分英文大小写，并要求缩写和关键词满足完整单词边界。匹配前会统一连字符、Unicode 破折号和连续空白；较宽泛的表征词可通过上下文门槛，要求同时命中指定材料或器件方向。自动标签仅供初步筛选，可能漏标或误标，不是编辑或人工标注结论。

每次 `update` 都会重新分类全部已存论文，包括 RSS 返回 304 未修改或没有新论文的运行；新关系会在同一个事务中整体替换，失败则保留原有标签。因此，修改词表后旧论文也会采用新规则。

日常维护流程：

1. 添加或修改 RSS 时编辑 `feeds.yml`；添加、删除或修改方向、精细标签、关键词或上下文门槛时编辑 `topics.yml`。
2. 重新生成 `docs/index.html` 中的“说明”区：
